...
**Без вопросов.** Заполните `.env` по образцу [.env.example](.env.example) и запустите `sudo ./deploy.sh </dev/null`. Если обязательного значения нет, скрипт выйдет с кодом 2. Шаги в панели он тогда только печатает и не ждёт вас. Ноду он запустит, если в `.env` уже есть `NODE_SECRET_KEY`; иначе на новом сервере проверка закончится кодом 8 на слое 1: создайте ноду в панели, впишите ключ и запустите скрипт ещё раз.

**Опрос в терминале** выглядит как форма: у каждого вопроса номер и подсказка, причина отказа выводится красным под полем, скрытый токен печатается точками, чтобы было видно, что вставка дошла. В лог и в конвейер идёт простой текст без цвета, а переменная `NO_COLOR` выключает цвет и в терминале.
...
| **Сервер** | Ubuntu 22.04/24.04/26.04 или Debian 12, доступ root. Ноду `remnanode` скрипт ставит сам на шаге 2 в панели, как в [официальной инструкции](https://docs.rw/install/remnawave-node/): Docker из get.docker.com, compose-файл в `/opt/remnanode` |
...
| **Firewall** | 8444/tcp для CDN, 443/tcp и 443/udp для Reality и Hysteria2, `NODE_PORT` (2222/tcp) для панели, 80/tcp в режиме `http-01` |
...
- **Нода, поставленная вручную,** отдаёт скрипту свои `SECRET_KEY` и `NODE_PORT` из `/opt/remnanode/docker-compose.yml`. Скрипт перепишет этот файл по образцу панели, с томом для Hysteria2, и один раз сохранит прежний рядом как `docker-compose.yml.cdn-deploy-orig`.
...
**Нода на этом сервере.** На шаге 2 скрипт берёт `SECRET_KEY` и `NODE_PORT` из `docker-compose.yml`, который панель показывает для новой ноды, пишет `/opt/remnanode/docker-compose.yml` с правами 600 и запускает контейнер. С `HY2_DOMAIN` в контейнер монтируется `/etc/letsencrypt`: без него xray не прочитает сертификат Hysteria2 и не стартует. Docker, если его нет, ставится из get.docker.com.
...
| 9 | `remnawave` | файлы для панели в `out/remnawave/` и пошаговая инструкция: профиль, нода с ним, панель, Timeweb; запуск ноды по её `SECRET_KEY` |
...
| `NODE_PORT` | `2222` | порт, на котором панель достучится до ноды; из `docker-compose.yml` панели |
| `NODE_SECRET_KEY` | — | `SECRET_KEY` ноды из того же файла; спрашивается один раз на шаге 2 в панели |
...
Конфиг xray на ноде ведёт панель, поэтому скрипт готовит файлы для неё и проводит по шагам. После генерации он печатает шесть шагов и в терминале ждёт Enter после каждого, а на шаге ноды спрашивает её `SECRET_KEY`. Проверка запускается уже после последнего шага. Профиль идёт первым: мастер «Создать ноду» без него не обойдётся.
...
2. **Нода.** Nodes → Management → Create node, адрес — IP сервера. На последнем шаге мастера выбрать профиль из шага 1 со всеми инбаундами → Create node. Панель покажет `docker-compose.yml`: вставьте в скрипт его `SECRET_KEY` (ввод скрыт, можно всю строку) и `NODE_PORT`. Скрипт запишет `/opt/remnanode/docker-compose.yml` и запустит ноду. Нода уже есть в панели: карточка ноды → Change Profile → профиль из шага 1.
...
> [!IMPORTANT]
> Hysteria2 читает сертификат `/etc/letsencrypt/live/HY2_DOMAIN/` внутри контейнера ноды, поэтому скрипт монтирует туда `/etc/letsencrypt:/etc/letsencrypt:ro`. Если compose-файл ноды ведёте вы сами, добавьте этот том и выполните `docker compose up -d`: без него xray на ноде не запустится с этим профилем. Сам сертификат к шагу 2 уже выпущен.
...
|---|---|---|
| **1. xray** | `127.0.0.1:XHTTP_PORT` принимает соединения | см. ниже |
...
**Если падает слой 1.** Запущенной ноде скрипт даёт до минуты: панель запускает xray, когда достучится до ноды. Если порт так и не открылся, он называет причину:
- **нет Docker или контейнера `remnanode`:** нода ещё не создана, или скрипт не получил её `SECRET_KEY`;
- **контейнер не видит `/etc/letsencrypt`:** xray падает на сертификате Hysteria2, повторный запуск перепишет compose-файл с томом;
- **ошибка из лога ноды:** последнее звено цепочки `Failed to start Xray`, например `open /etc/letsencrypt/live/…/fullchain.pem: no such file or directory`;
- **нода отвергла `SECRET_KEY`:** скопируйте его из панели заново;
- **нода работает, а порта нет:** панель не достучалась до `NODE_PORT` или у ноды не включён xhttp-инбаунд.
...
**Лимит устройств.** Если у пользователя в панели лимит устройств, Remnawave на запрос без `x-hwid` отдаёт вместо серверов заглушки вроде «Клиент не поддерживается». Поэтому чекер представляется одним устройством на машину: `x-hwid` из хеша `/etc/machine-id`, в списке устройств пользователя он виден как «cdn-deploy check.sh» и занимает одно место в лимите. Если мест нет, чекер скажет об этом и выйдет с кодом 1. Для регулярных проверок удобнее отдельный пользователь без лимита устройств.
...
| `CHECK_HWID` | один на машину | `x-hwid` запроса подписки; пустое значение не отправляет его |
